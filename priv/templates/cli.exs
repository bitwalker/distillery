defmodule Mix.Releases.Runtime.CLI do
  ...
  defmodule Log do
    ...
    def configure(%{verbosity: verbosity} = opts) do
      :ets.insert(__MODULE__, {:level, verbosity}); opts
    end
  ...
  def main() do
    ...
    args =
      ...
      |> Enum.drop_while(fn "--" -> false; _ -> true end)
      |> Enum.drop(1)
  ...
  def unpack_release(args, %{remote: remote}) do
    ...
    case List.keyfind(releases, version, 0) do
      nil ->
        ...
        Log.info "Release #{release}:#{version} not found, attempting to unpack releases/#{version}/#{release}.tar.gz"
  ...
  def install_release(args, %{remote: remote}) do
    ...
    case List.keyfind(releases, version, 0) do
      ...
      {_ver, :unpacked} ->
        ...
        install_and_permafy(remote, release, version)
      {_ver, :current} ->
        Log.info "Release #{release}:#{version} is already installed and current, making permanent.."
  ...
  defp parse_opts([{:name, name} | rest], opts) do
    dist_opts =
      case Regex.split(~r/@/, name) do
        ...
        [sname, host] ->
          {String.to_atom("#{sname}@#{host}"), String.to_atom("#{sname}_maint_@#{host}"), :longnames}
        _parts ->
  ...
  defp failed_connect!(remote) do
    Log.error "Received 'pang' from #{remote}. " <>
  ...
  defp install_and_permafy(remote, release, version) do
    ...
    case rpc(remote, :release_handler, :check_install_release, [vsn], :infinity) do
      ...
      {:ok, _other_vsn, _desc} ->
        :ok
      {:error, reason} ->
        Log.error "Release handler check for #{release}:#{version} failed with: #{inspect reason}"
    end
  ...
  defp update_config(remote) do
    ...
    with {:ok, providers} <- rpc(remote, :application, :get_env, [:distillery, :config_providers]),
  ...
  defp fix_release({:release, _release, _version, erts_vsn, libs, _status} = rel, root_dir, erts_dir) do
    ...
    cond do
      ...
      :else ->
        ...
        Log.error "Invalid RELEASES file: The specified ERTS #{erts_vsn} does not match host #{current_erts_vsn}"
    end
  end
